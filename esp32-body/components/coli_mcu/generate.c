                                         ...
                                         coli_generate_result_t *result)
{
    ...
    kv_cache = generate_alloc((size_t)kv_layout.total_bytes);
    ...
    status = coli_olmoe_generate_greedy(
    ...
    for (size_t i = prompt_count; i < output_count; ++i) {
        ...
        status = coli_tokenizer_decode(&tokenizer, &output_ids[i], 1, decoded,
                                       config->decoded_chunk_bytes,
                                       &decoded_bytes);
        if (status != COLI_OK) goto cleanup;
        result->decoded_bytes += decoded_bytes;
        if (config->log_chunk && decoded_bytes)
            config->log_chunk(config->callback_context, decoded, decoded_bytes);
        maybe_yield(config);
        if (cancelled(config)) {
            status = COLI_ERR_REMOVED;
            result->stage = COLI_GENERATE_STAGE_CANCELLED;
