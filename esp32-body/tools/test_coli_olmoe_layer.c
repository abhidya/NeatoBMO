...
static void write_fixture(const char *path)
{
    ...
    append_dense(entries, &count, coli_olmoe_post_attention_norm_id(0), &offset,
                 "postnorm");
    append_q4_pair(entries, &count, coli_olmoe_q_proj_id(0), HIDDEN, HIDDEN,
                   &offset, "q");
    append_q4_pair(entries, &count, coli_olmoe_k_proj_id(0), HIDDEN, HIDDEN,
                   &offset, "k");
    append_q4_pair(entries, &count, coli_olmoe_v_proj_id(0), HIDDEN, HIDDEN,
                   &offset, "v");
    append_q4_pair(entries, &count, coli_olmoe_o_proj_id(0), HIDDEN, HIDDEN,
                   &offset, "o");
    append_q4_pair(entries, &count, coli_olmoe_router_id(0), EXPERTS, HIDDEN,
                   &offset, "router");
    for (uint32_t expert = 0; expert < EXPERTS; ++expert) {
        append_q4_pair(entries, &count, coli_olmoe_expert_gate_id(0, expert),
                       HIDDEN, HIDDEN, &offset, "gate");
        append_q4_pair(entries, &count, coli_olmoe_expert_up_id(0, expert),
                       HIDDEN, HIDDEN, &offset, "up");
        append_q4_pair(entries, &count, coli_olmoe_expert_down_id(0, expert),
                       HIDDEN, HIDDEN, &offset, "down");
    }
...
static float rms_norm_scale(const float *x)
{
    double sum = 0.0;
    for (size_t i = 0; i < HIDDEN; ++i) sum += (double)x[i] * x[i];
    return (float)(1.0 / sqrt(sum / HIDDEN + 1.0e-5));
}
...
int main(void)
{
    char path[] = "/tmp/coli-olmoe-layer-XXXXXX";
    int fd = mkstemp(path);
    assert(fd >= 0);
    close(fd);
    write_fixture(path);
    ...
    float post[HIDDEN];
    float scale = rms_norm_scale(input);
    for (size_t i = 0; i < HIDDEN; ++i) {
        norm[i] = input[i] * scale;
        residual[i] = input[i] + norm[i];
    }
    scale = rms_norm_scale(residual);
    for (size_t i = 0; i < HIDDEN; ++i) post[i] = residual[i] * scale;
    for (size_t i = 0; i < HIDDEN; ++i) {
        float expected = residual[i] + silu(post[i]) * post[i];
        assert(fabsf(output[i] - expected) < 0.0005f);
    }
    float *cached_key = (float *)kv_cache;
    for (size_t i = 0; i < HIDDEN; ++i)
        assert(fabsf(cached_key[i] - norm[i]) < 0.0005f);
    ...
    unlink(path);
    puts("OLMoE single layer decode: PASS");
    return 0;
}
