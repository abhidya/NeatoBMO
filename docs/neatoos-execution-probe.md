...
| E12 | stock 2.5 with header `0x18` bit 0 flipped | `Upload code noburn` | terminator; identity healthy before and after | 805,892 bytes received; `NoWrite`; NAND failure | Transport complete; integrity untested |
| E13 | stock 2.5 with header `0x18` bit 0 flipped | `Upload code reboot` | ENQ then ACK; factory 2.4 returned | 805,892 bytes written OK; `Checksum error in application binary`; factory fallback | Header field is integrity-relevant |
| E14 | exact stock Cruz-P 2.5 | `Upload code reboot` | ENQ then ACK; USB identity `2.5.15893` | 805,892 bytes written OK; installed NEROS 15893 started | Recovery verified |
...
## E12-E13 opaque-header bit result

Four authentic Cruz-P images have a unique 16-byte clear-header field at
offsets `0x10..0x1f`. Offline comparison found that field is not the CBC IV and
does not equal common unkeyed checksums or digests over the obvious ciphertext
ranges. E12-E13 derived a probe only from exact stock Cruz-P 2.5 and changed one
bit while preserving every encrypted body byte:

```text
base sha256     e1a31ef56e2b617a4056d70c308aab26b7e2cd95e679cb982d697ff4f089c697
probe sha256    3c5440920ef0183a573b9c3071973cbf9c202faaf7561bb01d2c2e0053a87d3c
mutation        offset 0x18 XOR 0x01
body 0x200..EOF byte-identical to stock
```

E12 completed the no-write transport control with stock 2.5 healthy before and
after. E13 then wrote the same one-bit probe once. USB returned ACK, NAND region
`0x10000` reported a successful full write, and the bootstrap printed
`Checksum error in application binary` before loading factory NEROS 15667.

This proves that the `0x10..0x1f` field is checked by, or is an input to, the
application-integrity path. It does **not** prove that offset `0x18` literally
stores a checksum, distinguish checksum from MAC/nonce/keying metadata, identify
the algorithm, or test body encryption. E14 immediately restored exact stock;
P6 started NEROS build 15893 and USB confirmed software `2.5.15893`, mainboard
7.1.
...
- `captures/20260812_E12_header_field_bitflip_noburn_p6.log`
- `captures/20260812_E12_header_field_bitflip_noburn_usb.json`
- `captures/20260812_E13_header_field_bitflip_burn_p6.log`
- `captures/20260812_E13_header_field_bitflip_burn_usb.json`
- `captures/20260812_E14_stock_25_restore_p6.log`
- `captures/20260812_E14_stock_25_restore_usb.json`
...
1. Identify how the integrity-relevant `0x10..0x1f` field is generated by
   comparing multiple exact Cruz-P images or analyzing a lawful bootstrap
   readout. Do not infer that it is a simple checksum; common unkeyed candidates
   failed and the bit test proves participation, not semantics.
